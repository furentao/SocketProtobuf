...
class ThreadedServer(object):
    ...
    def handleTCPClientConnection(self, clientConnection, clientAddress):
		...
		if messageFromServer:
			print("MESSAGE FROM IP %s: %s"%(clientAddress[0], parseSerializesStringIntoMessageObj(messageFromServer)))

			# Handle the message according to received signal and send a response to server
			response = handleMessageAndGetResponse(messageFromServer)
...
def sendLampSensorData(ip, port):
	...
	print ("SERVER SAID: %s"%(parseSerializesStringIntoMessageObj(responseFromServer)))
...
def findServerOnTheInternet(ip, port):
	...
	print ("SERVER SAID: %s"%(parseSerializesStringIntoMessageObj(response)))
...
def parseSerializesStringIntoMessageObj(serialized_message_string):
    message = message_pb2.Message()
    message.ParseFromString(serialized_message_string)
    return message

def main():
	# ligar servidor multicast
